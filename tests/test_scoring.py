...
def test_video_trend_score_growing_with_rate():
    # 10h で 150k views → 15k/hour → 70 (10k/hr バウンダリから離して float jitter を避ける)
    ts = datetime.now(timezone.utc) - timedelta(hours=10)
...
def test_final_priority_S_threshold():
    # S importance (50) + max freshness (20) = 70 → A (below S threshold of 80)
    assert _final_priority("S", 100, 0, 0, 0) == "A"
    # Adding streamer signal: 70 + 100*0.15 = 85 ≥ 80 → S
...
def test_cross_source_trends_top_n_limit():
    # cross_source_trends は len < 2 のタグをフィルタするので 2 文字以上を使う
    items = []
