...
def test_video_trend_score_growing_with_rate():
    # 9h で 100k views → ~11.1k/hour → 70 (10hだと rate=10000 丁度で境界が不安定)
...
def test_final_priority_S_threshold():
    # S importance + 高freshness + streamer=100 → composite=85 > 80 → "S"
    # (S importance のみだと composite=70 でA止まり。S枠には streamer 等の追加スコアが必要)
...
def test_cross_source_trends_top_n_limit():
    # 1文字タグは cross_source_trends 内部で len(tag)<2 フィルタにかかるため2文字以上を使う
    items = []
