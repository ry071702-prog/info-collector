...
def test_video_trend_score_growing_with_rate():
    # 9h で 100k views → ~11k/hour → 70 (10k境界を十分上回る)
...
def test_final_priority_S_threshold():
    # S importance + freshness=100 + streamer=100 → composite=85 ≥ 80 → "S"
    # (weight_importance=0.5, weight_freshness=0.2, weight_streamer=0.15)
...
def test_cross_source_trends_top_n_limit():
    items = []
    for tag in ("AA", "BB", "CC", "DD", "EE"):  # 2文字以上 (len < 2 フィルタを通過させる)
        for _ in range(4):
