...
- 有料 LLM サービスや新しい外部課金サービスを追加しないでください (無料枠サービスは可)。
  - **例外 (許容済み)**: 新聞風画像生成 (`scripts/generate_newspaper_image.py`, `process_digest.yml`) は有料の画像モデル `gemini-3-pro-image-preview` を使う。1日1枚=月 ¥600〜1,100 目安で意図的に許容している。コスト最適化や無料枠厳守を理由にこの機能を勝手に無効化・モデル降格しないこと (変えるなら repo variable `NEWSPAPER_IMAGE_MODEL` で切替可、判断はユーザー)。
- `src/claude_client.py` は互換用で、実体は `src/llm_client.py` です。
- 個人運用向け。自動フィードの閲覧は誰でも可。「自分で保存→AI要約」(`/collection`) は **パスコード本人限定**。複雑な認証・ロール管理はなし。
- サイトには無料 Cloudflare バックエンドあり: **Pages Functions** (`site/functions/api/*`) + **D1** (`info-collector-saves`, binding `DB`, 設定は `site/wrangler.toml`)。保存時に runtime で Gemini を呼ぶ (`site/functions/_lib/gemini.ts`)。runtime secrets は Cloudflare Pages に `GEMINI_API_KEY` と `SAVE_PASSCODE`。`publish_site.yml` は `site/` で `wrangler pages deploy` する (Functions+D1 同梱)。
...
- 例外: 新聞風画像生成のみ有料モデルを許容 (「重要な前提」参照)。テキスト/分類パイプライン本体は無料枠厳守。
