...
- [x] 1. GitHubリポジトリ作成
- [x] 2. Google Gemini APIキー取得（無料・カード不要）
- [ ] 3. X (Twitter) 捨てアカウント作成 ← **🚧 後回し中（X_ACCOUNTS=[] で先に運用開始）**
...
## 📌 後で対応する: X (Twitter) 収集の有効化

初期セットアップでは X 収集を **後回し** にして起動しました（YouTube + Twitch + RSS のみで運用中）。
X を有効化するときは以下の手順:

1. **X 捨てアカウントを 2〜3個作成**（チェックリスト3の手順に従う）
   - SMS認証必要、Gmailの `+` トリックでメアド分離、専用パスワード使用
   - twscrape の `email_password` には Gmail なら**アプリパスワード**が安全
2. **JSON配列を組み立てる**:
   ```json
   [
     {"username":"acc1","password":"...","email":"your+x1@gmail.com","email_password":"..."},
     {"username":"acc2","password":"...","email":"your+x2@gmail.com","email_password":"..."}
   ]
   ```
3. **GitHub Secrets を更新**:
   ```bash
   gh secret set X_ACCOUNTS  # プロンプトでJSONを貼り付け（履歴に残らない）
   ```
4. **ローカル `.env` も更新**（DRY_RUN テスト用、コミット禁止）
5. **動作確認**: Actions タブから `Collect Realtime` を手動キック → ログで X 件数 > 0 を確認
6. **チェックリスト3に `[x]`** を入れる（このSETUP.md上部）

`src/collectors/x_twscrape.py` は **X_ACCOUNTS が空なら INFO ログでスキップ**する fallback 入りなので、
未設定のままでも他のコレクターは正常動作します。
