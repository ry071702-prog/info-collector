...
def main() -> None:
    # Process yesterday's raw data (Tokyo time → UTC offset already handled by GitHub Actions cron)
    target = datetime.utcnow() - timedelta(hours=12)
    date_str = target.strftime("%Y-%m-%d")
    log.info(f"Processing date: {date_str}")

    raw_items = list(read_raw(date_str))
